...
public class LeaderStateModel {
     ...
     */
    private Map<Node, AtomicLong> nextIndex = new ConcurrentHashMap<>();
    /**
    ...
    public LeaderStateModel(long index) {
        NodeGroup.getNodeGroup().forEach(node -> {
            //初始值为领导者最后的日志条目的索引+1
            nextIndex.put(node, new AtomicLong(index + 1));
            //初始值为0
    ...
    public long getNext(Node node) {
        return nextIndex.get(node).get();
    }

    public void setNextIndexForNode(Node node, long index) {
        nextIndex.get(node).set(index);
    }

    public void setNextIndexForNode(Node node, long old, long update) {
        if (!nextIndex.get(node).compareAndSet(old, update)) {
            log.info("设置失败：{},{}", old, update);
        }
    }
